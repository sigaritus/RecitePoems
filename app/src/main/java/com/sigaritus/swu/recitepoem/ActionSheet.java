...
public class ActionSheet extends Fragment implements OnClickListener {
	...
	private void createItems() {
		...
		if (titles != null) {
			for (int i = 0; i < titles.length; i++) {
				...
				bt.setId(R.id.CANCEL_BUTTON_ID + i + 1);
		...
		bt.setTextSize(TypedValue.COMPLEX_UNIT_PX, mAttrs.actionSheetTextSize);
		bt.setId(R.id.CANCEL_BUTTON_ID );
	...
	public void onClick(View v) {
		...
		dismiss();
		if (v.getId() != R.id.CANCEL_BUTTON_ID  && v.getId() != R.id.BG_VIEW_ID) {
			if (mListener != null) {
				mListener.onOtherButtonClick(this, v.getId() - R.id.CANCEL_BUTTON_ID
                        - 1);
			}
			isCancel = false;
		}
